...
\item Each value in the 9 dimensional output is a probability value representing the best action to take on the board (e.g. $3\times3$ actions) - i.e. the higher the number, the higher the probability of that action being the best action.  This policy is stochastic as it outputs a probability distribution of the best moves based on the current status of the board and not deterministic (e.g. there is no one best move for the current board status).
\end{enumerate}
\end{homeworkProblem}
